...
class Logger {
public:
	Logger(const std::string & format = DEFAULT_FORMAT): format_(format) {
	}
	virtual ~Logger() {
	}
...
public:
	OStreamLogger(std::ostream &stream, const std::string &format = DEFAULT_FORMAT):
	  Logger(format),
	  stream_(stream) {
	}
...
public:
	FileLogger(const std::string &fileName, const std::string &format = DEFAULT_FORMAT) : 
	  Logger(format), 
      fileName_(fileName) {
		fd_ = -1;
	}
