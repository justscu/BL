...
public:
	...
	bool init() {
		...
		listener_ = evconnlistener_new_bind(event_base_, cb_listen, (void*)this, LEV_OPT_REUSEABLE | LEV_OPT_CLOSE_ON_FREE,
		...
		// 信号处理
		signal_event_ = evsignal_new(event_base_, SIGINT, cb_SIGINT, (void*)this);
		if (signal_event_ == nullptr) {
...
private:
	...
	void accept(Thread* t, evutil_socket_t clientfd, struct sockaddr* addr, int /*socklen*/) {
		sockaddr_in* peer = (sockaddr_in*)addr;
		char buf[32];
		sprintf(buf, "%s:%d", inet_ntoa(peer->sin_addr), htons(peer->sin_port));
		printf("[%lu] accept: [%s] fd[%d]\n", t->get_thread_id(), buf, clientfd);

		struct bufferevent* bev = bufferevent_socket_new(t->get_event_base(), clientfd, BEV_OPT_CLOSE_ON_FREE);
		if (bev == nullptr) {
			printf("bufferevent_socket_new failed \n");
			return;
		}
		Session* s = new SESSION(t->get_thread_id(), clientfd, bev, std::string(buf));
		bufferevent_setcb (bev, ServerBase1<SESSION>::cb_read, nullptr, ServerBase1<SESSION>::cb_event, (void*)s);
...
private:
	...
	static void cb_event (struct bufferevent * /*bev*/, short what, void *session)  {
		Session* s = (Session*)session;
		s->OnEvent(what);
	}
...
private:
	// 监听回调函数，@fd为新客户端的fd.
	static void cb_listen(evconnlistener* /*listener*/, int fd, sockaddr* saddr, int socklen, void* arg) {
		((ServerBase1<SESSION>*)arg)->th_pool_->dispatch(std::bind(&ServerBase1<SESSION>::accept,
				(ServerBase1<SESSION>*)arg, std::placeholders::_1, fd, saddr, socklen));
	}
...
public:
	bool init() {
		...
		// set reuse
		int reuse = 1;
		if (0 != setsockopt(listenfd_, SOL_SOCKET, SO_REUSEADDR, (const void*)&reuse, sizeof(reuse))) {
...
//		addr.sin_addr.s_addr = inet_addr("0.0.0.0");
		addr.sin_addr.s_addr = htonl(INADDR_ANY);
		addr.sin_port        = htons(port_);
		if (0 != ::bind(listenfd_, (struct sockaddr*)&addr, sizeof(sockaddr))) {
			printf("::bind failed \n");
			return false;
		}
		...
		listen_event_ = event_new(event_base_, listenfd_, EV_READ|EV_PERSIST, cb_listen, this);
		if (listen_event_ == nullptr) {
...
private:
	...
	static void cb_listen (evutil_socket_t fd, short event, void * arg) {
		...
		((ServerBase2*)arg)->th_pool_->dispatch(std::bind(&ServerBase2<SESSION>::accept,
				(ServerBase2*)arg, std::placeholders::_1, fd, event));
	}
	static void cb_SIGINT(int sig, short int, void * arg) {
