...
public:
    ...
    static int64_t today_us1(const char *us) {
        int64_t t1 = 0;
    ...
    // ms: 20220607121338123 -> 121338123000
    static int64_t today_us2(const char *ms) {
        int64_t t1 = 0;
        t1 += ((ms[ 8]-'0') * 10 + (ms[ 9]-'0')) * 3600;
        t1 += ((ms[10]-'0') * 10 + (ms[11]-'0')) *   60;
        t1 += ( ms[12]-'0') * 10 + (ms[13]-'0');

        return t1 *1000000 + atoi(ms+14)*1000;
    }
};
