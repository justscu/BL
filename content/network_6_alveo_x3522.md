...
port_rx_nodesc_drops  | Number of packets dropped by the network adapter because of a lack of RX descriptors in the RX queue. Packets can be dropped by the NIC when there are insufficient(信息不足) RX descriptors in the RX queue to allocate to the packet. This problem occurs if the receive rate is very high and the network adapter receive cycle process has insufficient time between processing to refill the queue with new descriptors. <br/> A number of different steps can be tried to resolve this issue:<br/>Disable the irqbalance daemon in the OS. Distribute the traffic load across the available CPU/cores by setting rss_cpus=cores. Refer to Receive Side Scaling section. Increase receive queue size using ethtool.
