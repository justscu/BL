...
SSE指令要求数据块16字节对齐，即SSE单元和内存之间传输数据的指令要求内存地址必须是16的整数倍。
...
可以把xmm寄存器(128bit)看成一个数组，4个32bit[x32,x32,x32,x32]或2个64bit[x64,x64]。
...
浮点数传送指令
     指令   |   源  |  目的 |            描述                |     寄存器和内存     |
------------|:-----:|:-----:|--------------------------------|----------------------|
vmovss      | M32   | X     | 传送单精度                     | M32表示32bit内存     |
vmovss      | X     | M32   | 传送单精度                     | X表示寄存器xmm       |
vmovsd      | M64   | X     | 传送双精度                     | M64表示64bit内存     |
vmovsd      | X     | M64   | 传送双精度                     |
...
浮点数转换指令
   指令     |   源  | 目的|           描述                      |     寄存器和内存       |
------------|:-----:|:---:|-------------------------------------|------------------------|
vcvttss2si  | X/M32 | R32 | 用截断的方法把单精度数转成整数      | R32表示32bit通用寄存器 |
vcvttsd2si  | X/M64 | R32 | 用截断的方法把双精度数转成整数      |
vcvttss2siq | X/M32 | R64 | 用截断的方法把单精度数转成64bit整数 | 
vcvttsd2siq | X/M64 | R64 | 用截断的方法把双精度数转成64bit整数 |

浮点数转换指令
     指令   |    源1  | 源2  | 目的|        描述           |
------------|:-------:|:----:|:---:|-----------------------|
vcvtsi2ss   | M32/R32 |  X   |  X  | 把32bit整数转成单精度 |
vcvtsi2sd   | M32/R32 |  X   |  X  | 把32bit整数转成双精度 |
vcvtsi2ssq  | M64/R64 |  X   |  X  | 把64bit整数转成单精度 |
vcvtsi2sdq  | M64/R64 |  X   |  X  | 把64bit整数转成双精度 |
